...
import type {
  MembershipReportAuthorization,
  MembershipReportRouteServices,
} from "./membership-report-route-handlers";
...
async function authorize(permissionCode: string): Promise<MembershipReportAuthorization> {
...
export function createMembershipReportRouteServices(): MembershipReportRouteServices {
  return {
    authorize,
    listMembershipReport: (tenantId) => reportService.listMembershipReport({ tenantId }),
  };
}
