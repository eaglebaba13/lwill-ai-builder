...
import type {
  BranchPerformanceAuthorization,
  BranchPerformanceRouteServices,
} from "./branch-performance-route-handlers";
...
async function authorize(permissionCode: string): Promise<BranchPerformanceAuthorization> {
...
export function createBranchPerformanceRouteServices(): BranchPerformanceRouteServices {
  return {
    authorize,
    listBranchPerformance: (tenantId) => reportService.listBranchPerformance({ tenantId }),
  };
}
