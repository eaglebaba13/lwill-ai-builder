...
import type {
  DailySalesAuthorization,
  DailySalesRouteServices,
} from "./daily-sales-route-handlers";
...
async function authorize(permissionCode: string): Promise<DailySalesAuthorization> {
...
export function createDailySalesRouteServices(): DailySalesRouteServices {
  return {
    authorize,
    listDailySales: (tenantId) => reportService.listDailySales({ tenantId }),
  };
}
