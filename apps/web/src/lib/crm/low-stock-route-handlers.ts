...
export type LowStockAuthorization =
  | { readonly outcome: "unauthenticated" }
  | { readonly outcome: "forbidden" }
  | { readonly outcome: "authorized"; readonly tenantId: string };

export interface LowStockRouteServices {
  readonly authorize: (permissionCode: string) => Promise<LowStockAuthorization>;
  readonly listLowStockItems: (tenantId: string) => Promise<readonly unknown[]>;
}
...
function authorizationOutcome(
  authorization: LowStockAuthorization,
...
export async function handleGetLowStockItems(
  _request: Request,
  services: LowStockRouteServices,
): Promise<Response> {
  ...
  const items = await services.listLowStockItems(authResult.tenantId);
  return response(200, { lowStockItems: items });
}
