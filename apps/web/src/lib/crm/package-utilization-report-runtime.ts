...
import type {
  PackageUtilizationReportAuthorization,
  PackageUtilizationReportRouteServices,
} from "./package-utilization-report-route-handlers";
...
async function authorize(permissionCode: string): Promise<PackageUtilizationReportAuthorization> {
...
export function createPackageUtilizationReportRouteServices(): PackageUtilizationReportRouteServices {
  return {
    authorize,
    listPackageUtilizationReport: (tenantId) => reportService.listPackageUtilizationReport({ tenantId }),
  };
}
