...
function createFixture(overrides: {
  ...
  const prisma: ServicePermissionsBootstrapPrismaClient = {
    $transaction: vi.fn(async (callback) => callback(transaction as never)),
  };
  return { prisma, state, transaction };
}

describe("service permissions bootstrap", () => {
  it("creates service.read and service.write permissions and assigns them to tenant-admin", async () => {
    const fixture = createFixture();

    const result = await bootstrapServicePermissions(fixture.prisma);

    expect(result).toMatchObject({
      tenantId: "tenant-1",
      roleId: "role-1",
      permissionCodes: ["service.read", "service.write"],
  ...
  it("is idempotent when permissions and role permissions already exist", async () => {
    const fixture = createFixture({
      roleMatches: [{
        ...
        isActive: true,
        permissions: [
          { permission: { code: "service.read" } },
          { permission: { code: "service.write" } },
        ],
      }],
    });
    fixture.state.permissions.set("service.read", { id: "perm-1", code: "service.read" });
    fixture.state.permissions.set("service.write", { id: "perm-2", code: "service.write" });
  ...
  it("fails closed when the target tenant is missing", async () => {
    ...
    await expect(bootstrapServicePermissions(fixture.prisma)).rejects.toThrow(
