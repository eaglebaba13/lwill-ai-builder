...
import {
  ...
  ProductPermissionsBootstrapError,
...
function createFixture(overrides: {
  ...
  const transaction: ProductPermissionsBootstrapPrismaClient = {
            ...
            return [
              {
                ...
                permissions: overrides.preexistingPermissions
                  ? [{ permission: { code: "product.read" } }, { permission: { code: "product.write" } }]
                  : [],
              },
            ];
          }),
...
describe("product permissions bootstrap", () => {
  it("creates product.read and product.write permissions and assigns them to tenant-admin", async () => {
    const { transaction, state } = createFixture();

    const result = await bootstrapProductPermissions(transaction);

    expect(result).toMatchObject({
      tenantId: "tenant-1",
      roleId: "role-1",
      permissionCodes: ["product.read", "product.write"],
      permissionsCreated: 2,
      rolePermissionsCreated: 2,
    });
    expect(state.permissions.get("product.read")).toBe("perm-product.read");
    expect(state.permissions.get("product.write")).toBe("perm-product.write");
  });
  ...
  it("is idempotent when permissions and role permissions already exist", async () => {
    ...
    state.permissions.set("product.read", "perm-product.read");
    state.permissions.set("product.write", "perm-product.write");
  ...
  it("fails closed when the target tenant is missing", async () => {
    ...
    await expect(bootstrapProductPermissions(transaction)).rejects.toThrow(
  ...
  it("does not return or log sensitive data", () => {
    const result = formatProductPermissionsBootstrapResult({
      tenantId: "tenant-1",
      roleId: "role-1",
      permissionCodes: ["product.read", "product.write"],
      permissionsCreated: 2,
    ...
    expect(formatProductPermissionsBootstrapError(new Error("secret"))).toBe(
