...
function createFixture(overrides: {
  ...
  const prisma: StockAdjustmentPermissionsBootstrapPrismaClient = {
    $transaction: vi.fn(async (callback) => callback(transaction as never)),
  };
  return { prisma, state, transaction };
}

describe("stock adjustment permissions bootstrap", () => {
  it("creates stockAdjustment.read and stockAdjustment.write permissions and assigns them to tenant-admin", async () => {
    const fixture = createFixture();

    const result = await bootstrapStockAdjustmentPermissions(fixture.prisma);

    expect(result).toMatchObject({
      tenantId: "tenant-1",
      roleId: "role-1",
      permissionCodes: ["stockAdjustment.read", "stockAdjustment.write"],
  ...
  it("is idempotent when permissions and role permissions already exist", async () => {
    const fixture = createFixture({
      roleMatches: [{
        ...
        isActive: true,
        permissions: [
          { permission: { code: "stockAdjustment.read" } },
          { permission: { code: "stockAdjustment.write" } },
        ],
      }],
    });
    fixture.state.permissions.set("stockAdjustment.read", { id: "perm-1", code: "stockAdjustment.read" });
    fixture.state.permissions.set("stockAdjustment.write", { id: "perm-2", code: "stockAdjustment.write" });
  ...
  it("fails when the target tenant is missing", async () => {
    ...
    await expect(bootstrapStockAdjustmentPermissions(fixture.prisma)).rejects.toThrow(
