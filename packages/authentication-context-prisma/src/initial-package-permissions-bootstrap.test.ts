...
function createFixture(overrides: {
  ...
  const prisma: PackagePermissionsBootstrapPrismaClient = {
    $transaction: vi.fn(async (callback) => callback(transaction as never)),
  };
  return { prisma, state, transaction };
}

describe("package permissions bootstrap", () => {
  it("creates package.read and package.write permissions and assigns them to tenant-admin", async () => {
    const fixture = createFixture();

    const result = await bootstrapPackagePermissions(fixture.prisma);

    expect(result).toMatchObject({
      tenantId: "tenant-1",
      roleId: "role-1",
      permissionCodes: ["package.read", "package.write"],
  ...
  it("is idempotent when permissions and role permissions already exist", async () => {
    ...
    fixture.state.permissions.set("package.read", { id: "perm-1", code: "package.read" });
    fixture.state.permissions.set("package.write", { id: "perm-2", code: "package.write" });
  ...
  it("fails closed when the target tenant is missing", async () => {
    ...
    await expect(bootstrapPackagePermissions(fixture.prisma)).rejects.toThrow(
