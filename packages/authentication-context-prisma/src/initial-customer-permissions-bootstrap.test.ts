...
function createFixture(overrides: {
  ...
  const prisma: CustomerPermissionsBootstrapPrismaClient = {
    $transaction: vi.fn(async (callback) => callback(transaction as never)),
  };
  return { prisma, state, transaction };
}

describe("customer permissions bootstrap", () => {
  it("creates customer.read and customer.write permissions and assigns them to tenant-admin", async () => {
    const fixture = createFixture();

    const result = await bootstrapCustomerPermissions(fixture.prisma);

    expect(result).toMatchObject({
      tenantId: "tenant-1",
      roleId: "role-1",
      permissionCodes: ["customer.read", "customer.write"],
  ...
  it("is idempotent when permissions and role permissions already exist", async () => {
    ...
    fixture.state.permissions.set("customer.read", { id: "perm-1", code: "customer.read" });
    fixture.state.permissions.set("customer.write", { id: "perm-2", code: "customer.write" });
  ...
  it("fails closed when the target tenant is missing", async () => {
    ...
    await expect(bootstrapCustomerPermissions(fixture.prisma)).rejects.toThrow(
