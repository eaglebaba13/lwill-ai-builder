...
import {
  ...
  MembershipPermissionsBootstrapError,
...
function createFixture(overrides: {
  ...
  const transaction: MembershipPermissionsBootstrapPrismaClient = {
            ...
            return [
              {
                ...
                permissions: overrides.preexistingPermissions
                  ? [{ permission: { code: "membership.read" } }, { permission: { code: "membership.write" } }]
                  : [],
              },
            ];
          }),
...
describe("membership permissions bootstrap", () => {
  it("creates membership.read and membership.write permissions and assigns them to tenant-admin", async () => {
    const { transaction, state } = createFixture();

    const result = await bootstrapMembershipPermissions(transaction);

    expect(result).toMatchObject({
      tenantId: "tenant-1",
      roleId: "role-1",
      permissionCodes: ["membership.read", "membership.write"],
      permissionsCreated: 2,
      rolePermissionsCreated: 2,
    });
    expect(state.permissions.get("membership.read")).toBe("perm-membership.read");
    expect(state.permissions.get("membership.write")).toBe("perm-membership.write");
  });
  ...
  it("is idempotent when permissions and role permissions already exist", async () => {
    ...
    state.permissions.set("membership.read", "perm-membership.read");
    state.permissions.set("membership.write", "perm-membership.write");
  ...
  it("fails closed when the target tenant is missing", async () => {
    ...
    await expect(bootstrapMembershipPermissions(transaction)).rejects.toThrow(
  ...
  it("does not return or log sensitive data", () => {
    const result = formatMembershipPermissionsBootstrapResult({
      tenantId: "tenant-1",
      roleId: "role-1",
      permissionCodes: ["membership.read", "membership.write"],
      permissionsCreated: 2,
    ...
    expect(formatMembershipPermissionsBootstrapError(new Error("secret"))).toBe(
