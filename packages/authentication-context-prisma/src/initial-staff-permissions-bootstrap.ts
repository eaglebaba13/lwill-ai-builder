export const STAFF_PERMISSION_CODES = ["staff.read", "staff.write"] as const;
...
export class StaffPermissionsBootstrapError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "StaffPermissionsBootstrapError";
  }
}
...
interface StaffPermissionsTransactionClient {
  readonly tenant: {
    findMany(args: {
      where: { name: string };
...
export interface StaffPermissionsBootstrapPrismaClient {
  $transaction<T>(
    callback: (transaction: StaffPermissionsTransactionClient) => Promise<T>,
  ): Promise<T>;
}

export interface StaffPermissionsBootstrapResult {
...
export async function bootstrapStaffPermissions(
  prisma: StaffPermissionsBootstrapPrismaClient,
): Promise<StaffPermissionsBootstrapResult> {
  return prisma.$transaction(async (transaction) => {
    ...
    if (tenantMatches.length === 0) {
      throw new StaffPermissionsBootstrapError(
        "Target tenant not found; run the initial hierarchy bootstrap first",
      );
    }
    if (tenantMatches.length > 1) {
      throw new StaffPermissionsBootstrapError("Ambiguous target tenant records");
    }
    const tenant = tenantMatches[0]!;
    if (!tenant.isActive) {
      throw new StaffPermissionsBootstrapError("Target tenant is inactive");
    }
    ...
    if (roleMatches.length > 1) {
      throw new StaffPermissionsBootstrapError("Ambiguous target role records");
    }
    ...
    if (
      ...
      throw new StaffPermissionsBootstrapError("Conflicting target role record");
    }
...
export function formatStaffPermissionsBootstrapResult(
  result: StaffPermissionsBootstrapResult,
): string {
  return JSON.stringify({ status: "completed", ...result });
}

export function formatStaffPermissionsBootstrapError(error: unknown): string {
  return error instanceof StaffPermissionsBootstrapError
    ? error.message
    : "Staff permissions bootstrap failed";
}
