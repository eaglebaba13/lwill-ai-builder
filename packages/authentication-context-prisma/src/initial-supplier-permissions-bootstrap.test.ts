...
function createFixture(overrides: {
  ...
  const prisma: SupplierPermissionsBootstrapPrismaClient = {
    $transaction: vi.fn(async (callback) => callback(transaction as never)),
  };
  return { prisma, state, transaction };
}

describe("supplier permissions bootstrap", () => {
  it("creates supplier.read and supplier.write permissions and assigns them to tenant-admin", async () => {
    const fixture = createFixture();

    const result = await bootstrapSupplierPermissions(fixture.prisma);

    expect(result).toMatchObject({
      tenantId: "tenant-1",
      roleId: "role-1",
      permissionCodes: ["supplier.read", "supplier.write"],
  ...
  it("is idempotent when permissions and role permissions already exist", async () => {
    const fixture = createFixture({
      roleMatches: [{
        ...
        isActive: true,
        permissions: [
          { permission: { code: "supplier.read" } },
          { permission: { code: "supplier.write" } },
        ],
      }],
    });
    fixture.state.permissions.set("supplier.read", { id: "perm-1", code: "supplier.read" });
    fixture.state.permissions.set("supplier.write", { id: "perm-2", code: "supplier.write" });
  ...
  it("fails when the target tenant is missing", async () => {
    ...
    await expect(bootstrapSupplierPermissions(fixture.prisma)).rejects.toThrow(
