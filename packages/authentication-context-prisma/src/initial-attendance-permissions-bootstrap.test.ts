...
import {
  ...
  AttendancePermissionsBootstrapError,
...
function createFixture(overrides: {
  ...
  const transaction: AttendancePermissionsBootstrapPrismaClient = {
            ...
            return [
              {
                ...
                permissions: overrides.preexistingPermissions
                  ? [{ permission: { code: "attendance.read" } }, { permission: { code: "attendance.write" } }]
                  : [],
              },
            ];
          }),
...
describe("attendance permissions bootstrap", () => {
  it("creates attendance.read and attendance.write permissions and assigns them to tenant-admin", async () => {
    const { transaction, state } = createFixture();

    const result = await bootstrapAttendancePermissions(transaction);

    expect(result).toMatchObject({
      tenantId: "tenant-1",
      roleId: "role-1",
      permissionCodes: ["attendance.read", "attendance.write"],
      permissionsCreated: 2,
      rolePermissionsCreated: 2,
    });
    expect(state.permissions.get("attendance.read")).toBe("perm-attendance.read");
    expect(state.permissions.get("attendance.write")).toBe("perm-attendance.write");
  });
  ...
  it("is idempotent when permissions and role permissions already exist", async () => {
    ...
    state.permissions.set("attendance.read", "perm-attendance.read");
    state.permissions.set("attendance.write", "perm-attendance.write");
  ...
  it("fails closed when the target tenant is missing", async () => {
    ...
    await expect(bootstrapAttendancePermissions(transaction)).rejects.toThrow(
  ...
  it("does not return or log sensitive data", () => {
    const result = formatAttendancePermissionsBootstrapResult({
      tenantId: "tenant-1",
      roleId: "role-1",
      permissionCodes: ["attendance.read", "attendance.write"],
      permissionsCreated: 2,
    ...
    expect(formatAttendancePermissionsBootstrapError(new Error("secret"))).toBe(
