...
function createFixture(overrides: {
  ...
  const prisma: BranchPermissionsBootstrapPrismaClient = {
    $transaction: vi.fn(async (callback) => callback(transaction as never)),
  };
  return { prisma, state, transaction };
}

describe("branch permissions bootstrap", () => {
  it("creates branch.read and branch.write permissions and assigns them to tenant-admin", async () => {
    const fixture = createFixture();

    const result = await bootstrapBranchPermissions(fixture.prisma);

    expect(result).toMatchObject({
      tenantId: "tenant-1",
      roleId: "role-1",
      permissionCodes: ["branch.read", "branch.write"],
  ...
  it("is idempotent when permissions and role permissions already exist", async () => {
    const fixture = createFixture({
      roleMatches: [{
        ...
        isActive: true,
        permissions: [
          { permission: { code: "branch.read" } },
          { permission: { code: "branch.write" } },
        ],
      }],
    });
    fixture.state.permissions.set("branch.read", { id: "perm-1", code: "branch.read" });
    fixture.state.permissions.set("branch.write", { id: "perm-2", code: "branch.write" });
  ...
  it("fails closed when the target tenant is missing", async () => {
    ...
    await expect(bootstrapBranchPermissions(fixture.prisma)).rejects.toThrow(
