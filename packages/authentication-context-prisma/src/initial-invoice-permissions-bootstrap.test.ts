...
import {
  ...
  InvoicePermissionsBootstrapError,
...
function createFixture(overrides: {
  ...
  const transaction: InvoicePermissionsBootstrapPrismaClient = {
            ...
            return [
              {
                ...
                permissions: overrides.preexistingPermissions
                  ? [{ permission: { code: "invoice.read" } }, { permission: { code: "invoice.write" } }]
                  : [],
              },
            ];
          }),
...
describe("invoice permissions bootstrap", () => {
  it("creates invoice.read and invoice.write permissions and assigns them to tenant-admin", async () => {
    const { transaction, state } = createFixture();

    const result = await bootstrapInvoicePermissions(transaction);

    expect(result).toMatchObject({
      tenantId: "tenant-1",
      roleId: "role-1",
      permissionCodes: ["invoice.read", "invoice.write"],
      permissionsCreated: 2,
      rolePermissionsCreated: 2,
    });
    expect(state.permissions.get("invoice.read")).toBe("perm-invoice.read");
    expect(state.permissions.get("invoice.write")).toBe("perm-invoice.write");
  });
  ...
  it("is idempotent when permissions and role permissions already exist", async () => {
    ...
    state.permissions.set("invoice.read", "perm-invoice.read");
    state.permissions.set("invoice.write", "perm-invoice.write");
  ...
  it("fails closed when the target tenant is missing", async () => {
    ...
    await expect(bootstrapInvoicePermissions(transaction)).rejects.toThrow(
  ...
  it("does not return or log sensitive data", () => {
    const result = formatInvoicePermissionsBootstrapResult({
      tenantId: "tenant-1",
      roleId: "role-1",
      permissionCodes: ["invoice.read", "invoice.write"],
      permissionsCreated: 2,
    ...
    expect(formatInvoicePermissionsBootstrapError(new Error("secret"))).toBe(
