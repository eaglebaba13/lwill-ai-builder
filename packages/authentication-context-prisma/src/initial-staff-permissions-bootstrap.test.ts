...
import {
  ...
  StaffPermissionsBootstrapError,
...
function createFixture(overrides: {
  ...
  const transaction: StaffPermissionsBootstrapPrismaClient = {
            ...
            return [
              {
                ...
                permissions: overrides.preexistingPermissions
                  ? [{ permission: { code: "staff.read" } }, { permission: { code: "staff.write" } }]
                  : [],
              },
            ];
          }),
...
describe("staff permissions bootstrap", () => {
  it("creates staff.read and staff.write permissions and assigns them to tenant-admin", async () => {
    const { transaction, state } = createFixture();

    const result = await bootstrapStaffPermissions(transaction);

    expect(result).toMatchObject({
      tenantId: "tenant-1",
      roleId: "role-1",
      permissionCodes: ["staff.read", "staff.write"],
      permissionsCreated: 2,
      rolePermissionsCreated: 2,
    });
    expect(state.permissions.get("staff.read")).toBe("perm-staff.read");
    expect(state.permissions.get("staff.write")).toBe("perm-staff.write");
  });
  ...
  it("is idempotent when permissions and role permissions already exist", async () => {
    ...
    state.permissions.set("staff.read", "perm-staff.read");
    state.permissions.set("staff.write", "perm-staff.write");
  ...
  it("fails closed when the target tenant is missing", async () => {
    ...
    await expect(bootstrapStaffPermissions(transaction)).rejects.toThrow(
  ...
  it("does not return or log sensitive data", () => {
    const result = formatStaffPermissionsBootstrapResult({
      tenantId: "tenant-1",
      roleId: "role-1",
      permissionCodes: ["staff.read", "staff.write"],
      permissionsCreated: 2,
    ...
    expect(formatStaffPermissionsBootstrapError(new Error("secret"))).toBe(
